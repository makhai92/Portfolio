...
DigitalRain.prototype.getColor = function (
  ...
  if (
    (next && self.isCellLabel({ x, y, word: self.__word })) ||
    (self.isCellLabel({ x, y, word: self.options.word }) &&
      !next &&
      (!active || self.__iterations === 0))
  ) {
    self.context.shadowColor = "#00ff00"; /* 원래는 없없음 */
    self.context.shadowBlur = 15; /* 원래는 없었음 */
    return "white";
  }
...
DigitalRain.prototype.render = function () {
  ...
  for (let c = 0; c < self.characters; c++) {
    ...
    if (!self.active && !self.isCellLabel({ x, y, word: self.options.word }))
      continue;
    ...
    self.context.fillStyle = self.getColor(x, y, {
      ...column,
      low: column.complete ? 0 : 0.1,
    });
    ...
    if (chars[y] || self.isCellLabel({ x, y, word: self.options.word })) {
      ...
      if (
        self.isCellLabel({ x, y, word: self.options.word }) &&
        (!column.active || self.__iterations === 0)
      ) {
        let charIndex =
          x -
          (Math.floor((self.columns - 1) * 0.5) -
            self.options.word.length * 0.5);
        char =
          self.options.word.split("")[
            Number.isInteger(charIndex) ? charIndex - 1 : Math.floor(charIndex)
          ];

        // This part triggers it into reps and wipes out the first word
        if (
          self.active &&
          column.row === Math.floor(self.rows * 0.5) &&
          !column.complete
        ) {
          self.wiped += 1;
      ...
      if (
        self.__iterations === 0 &&
        (self.isCellLabel({ x, y, word: self.__word }) ||
          self.isCellLabel({ x, y, word: self.options.word })) &&
        column.row === Math.floor(self.rows * 0.5) &&
        !column.switch
      ) {
      ...
      if (column.next && self.isCellLabel({ x, y, word: self.options.word })) {
        let charIndex =
          x - (Math.floor((self.columns - 1) * 0.5) - self.__word.length * 0.5);
        char =
          self.__word.split("")[
            Number.isInteger(charIndex) ? charIndex - 1 : Math.floor(charIndex)
          ] || chars[y];
...
DigitalRain.prototype.switchWord = function (word, iterations = 1, callback) {
  ...
  for (let c = 0; c < self.columns; c++) {
    self.tracker[c] = self.setColumn(
      iterations === 0 ? { active: true, reps: 1 } : {}
    );
  }
  self.__word = word;
...
const SEND = () => {
  if (count % 2) {
    myDigitalRain.switchWord("SEND", 0, inc);
  } else {
    BUTTON.disabled = true;
    myDigitalRain.switchWord("SENDED", 3, inc);
  }
};
